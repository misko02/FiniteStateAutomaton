...
<p> Opens a dialog window allowing creating a new automaton using a graphical interface. </p>
...
<p>Allows you to load automaton from a txt file </p>
...
<p> Loads a predefined automaton from an example pool </p>

#### Automatons:

- ZipCodeAutomaton.txt—Automaton accepting Polish zip codes—Example of DFA
- BinaryAutomaton.txt—Automaton accepting binary strings with substrings "01" and "10"—Example of NFA
- numberParser.txt—Automaton accepting floating point numbers with or without a leading sign—Example of NFA with
  epsilon transitions
...
<p> In the beginning we have to choose the type of automaton we want to create (DFA for deterministic automaton, NFA for non-deterministic automaton). <br>
...
1. The first step will be to specify the number of states we want to add to the automaton
    * We are giving the number of states that we want to add to the automaton
    * For each state we will be able to specify its label (label means only the suffix of the state, we don't have to
      add "q", it will be automatically added).
2. The second step will be to specify the number of characters that our alphabet will have
    * In the beginning we declare the number of characters that our alphabet will have
...
3. The third step will be to specify the initial state of the automaton
    * In the beginning we declare the initial state of the automaton
        * _Warning:_ We have to give only the label of the state, additionally, the state must be in the set of states
          declared in the first step
...
4. The last step will be to specify the delta function
    * For each state and character of the alphabet, we specify the state to which the automaton goes after reading the
      given character
...
The procedure of loading non-deterministic automaton is very similar to loading deterministic automaton, with some
differences, which will be described below. <br>
Additionally, the procedure may slightly differ depending on whether we have chosen an automaton with epsilon
transitions or without.
...
2. The second step will be to specify the characters of the alphabet
    * If we have chosen an NFA with epsilon transitions, the epsilon character will be automatically added to the
      alphabet, without the need to declare it
...
4. The last step will be to specify a delta function
    * This step is the most different from loading a deterministic automaton
        * First, for each state and character of the alphabet, we can specify the number of states to which the
          automaton goes after reading the given character
        * Then, we choose the states to which the automaton goes after reading the given character
            * _Warning:_ All states must be in the set of states declared in the first step, and characters in the set
              of characters declared in the second step
            * _Warning 2:_ In the case of choosing an NFA with epsilon transitions, for each state and character of the
              alphabet, we can additionally specify the number of states to which the automaton goes after reading the
              epsilon character
            * _Warning 3:_ If we want the automaton to go to no state after a given character (i.e., go to an empty set
              of states), we enter "0".
5. After loading all information, the automaton will be saved and displayed in tabular form. Notice that now states are
   defined as sets.
...
Additionally, to avoid problems with displaying tables, it is recommended to run the program in the "Terminal" program
